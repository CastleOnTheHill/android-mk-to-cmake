...
def convert_ir(ir: dict[str, Any], output_rel: pathlib.Path) -> str:
    parts: list[str] = [
...
def main() -> int:
    ...
    for ir_path in sorted(ir_dir.glob("*.ir.json")):
        ...
        digest = input_hash([source_rel, ir.get("source_sha256", ""), sha256_file(ir_path)])
        if not args.force and manifest.done("convert_ir", ir_path.stem, digest, [output]):
            converted.append({"source": source_rel, "output": rel(output, root), "reused": True})
            continue
        cmake = convert_ir(ir, out_rel)
        atomic_write_text(output, cmake)
        manifest.mark("convert_ir", ir_path.stem, "done", digest, [output])
        converted.append({"source": source_rel, "output": rel(output, root), "reused": False})

    summary = state / "generated_manifest.json"
    atomic_write_json(summary, {"schema_version": 1, "files": converted})
    print(f"convert_ir: generated {len(converted)} CMake file(s)")
    return 0
